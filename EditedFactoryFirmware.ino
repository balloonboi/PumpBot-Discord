...
void loop() {
  ...
  if (data == "pump50" || data == "pump50\n" || data == "pump50\r\n" || data == "pump50\r") {
      pump50();
  }
  if (data.startsWith("pump")) {
    String pump1 = data.substring(5, data.indexOf(","));
    String pump2 = data.substring(data.indexOf(",") + 1, data.length());
    int pump1Int = pump1.toInt();
    int pump2Int = pump2.toInt();
    switchOnPumps(pump1Int);
    blow();
    delay(pump2Int * 1000);
    closeAllValves();
  }
  if (data.startsWith("vent")) {
    String venttime = data.substring(5, data.length());
    int venttimeInt = venttime.toInt();
    vent();
    delay(venttimeInt * 1000);
    closeAllValves();
  }
  }
